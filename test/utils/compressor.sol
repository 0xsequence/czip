...
library Compressor {
  ...
  function useStorage(CommandBuffer memory buffer) internal pure returns (CommandBuffer memory) {
    string[] memory inputs = new string[](buffer.commands.length + 1);
  ...
  function allowOps(CommandBuffer memory buffer, string memory ops) internal pure returns (CommandBuffer memory) {
    bytes32 aohash = keccak256(bytes("--allow-opcodes"));
    bool hasCommand = false;
    for (uint i = 0; i < buffer.commands.length; i++) {
      if (keccak256(bytes(buffer.commands[i])) == aohash) {
        hasCommand = true;
        break;
      }
    }
    
    string[] memory inputs = new string[](buffer.commands.length + (hasCommand ? 0 : 2));
